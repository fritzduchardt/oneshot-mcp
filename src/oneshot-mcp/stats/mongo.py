...
client = MongoClient(f"mongodb://{os.environ.get("MONGO_HOST", "localhost")}:{os.environ.get("MONGO_PORT", 27017)}")
db = client["oneshot"]
...
def insert_stats(collection:str, payload:str) -> bool:
    logging.info(f"Insert inti collection: {collection}, payload: {payload}")
...
def read_stats(collection:str, filters:str) -> Any | None:
